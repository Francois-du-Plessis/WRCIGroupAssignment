...
namespace NNPredictingRougthness
{
    class GreyImage
    {
        ...
        public GreyImage(string nem, Surface surface, List<double> scalePixelArray)
        {

            scaledPixelArray = scalePixelArray;     
            pathName = nem; // File path of image
            this.surface = surface;
        }
        ...
        {
            scaledPixelArray.Sort();
            byte[] pixels = new byte[10000];
            for (int x = 0; x < 10000; x++)
            {
                pixels[x] = Convert.ToByte(Math.Round((scaledPixelArray[x]) * 255));
            }
